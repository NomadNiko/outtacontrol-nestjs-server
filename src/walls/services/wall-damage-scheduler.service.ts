  ...
  async applyHourlyDamage() {
    ...
    try {
        ...
        try {
          ...
          if (damageResult.damage > 0) {
            ...
            if (damageResult.isDestroyed) {
              ...
              if (wall.fromFarm?.id) affectedFarmIds.add(String(wall.fromFarm.id));
      ...
      if (affectedFarmIds.size > 0) {
        this.logger.log(`Recalculating levels for ${affectedFarmIds.size} affected farms`);
        ...
        try {
          ...
          const extendedAffectedFarmIds = this.farmLevelService.getAffectedFarms(
          ...
          for (const [farmId, newLevel] of newLevels) {
            ...
            if (farm && farm.level !== newLevel) {
              ...
              this.logger.log(`Farm ${farmId} level changed from ${farm.level} to ${newLevel}`);
            }
          }
        } catch (error) {
