...
export class UsersDocumentRepository implements UserRepository {
  ...
  async update(id: User['id'], payload: Partial<User>): Promise<User | null> {
    console.log('🗄️ [USER REPO] Update called for user:', id);
    console.log('📋 [USER REPO] Payload received:', payload);
    ...
    if (!user) {
      console.log('❌ [USER REPO] User not found with ID:', id);
      return null;
    }

    console.log('👤 [USER REPO] Current user from DB:', {
      id: user._id,
    ...
    console.log('🔀 [USER REPO] Merged user object:', {
      id: mergedUser.id,
      platinum: mergedUser.platinum,
      gold: mergedUser.gold,
      silver: mergedUser.silver,
    });
    ...
    console.log('💚 [USER REPO] Updated user from DB:', {
      id: userObject?._id,
      platinum: userObject?.platinum,
